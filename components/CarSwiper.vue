<template>
  <ClientOnly>
    <div class="car-swiper relative 
      ...
      lg:pt-[1.4rem] lg:pb-[1.2rem] lg:px-0"
      :class="props.c5Slides.c5WrapperClassName">
      <!-- title & sub title -->
      <div 
        class="title-info font-miLight mx-auto lg:mb-[0.48rem] mb-[0.32rem]" 
        :style="infoStyle"
      ...
      >
        <div 
          ...
          class="text-[0.48rem] leading-[0.64rem]
            lg:text-[0.4rem] lg:leading-[0.53rem] text-[#222]"
        >{{ title }}</div>
        <div 
          v-if="subTitle" 
          class="mt-[0.24rem] text-[0.24rem] leading-[0.32rem] text-[#666]"
        >{{ subTitle }}</div>
      </div>
      <!-- swiper -->
      <swiper-container 
      ...
      >
        ...
        <div
          ...
          :style="infoStyle"
        >
          <span class="leading-[0.32rem] text-white tect-[0.24rem]">{{ activeSlide?.title }}</span>
          <div class="flex gap-x-[0.09rem] relative">
            <div 
              class="bar w-[0.48rem] h-[0.04rem] bg-white/30" 
              v-for="(_, idx) in slides" 
              :key="idx"
              @click="handleSlideTo(idx)"
            ></div>
            <div 
              class="active-bar w-[0.48rem] h-[0.04rem] bg-white/80 top-0 absolute duration-300"
      ...
      >
        ...
        <div 
          ...
          <div 
            class="tab-item flex-1 text-center leading-[0.32rem] text-[0.24rem] mb-[0.15rem] text-[#666] cursor-pointer"
        ...
        <div 
          ...
          class="lg:text-[#222] text-[#999] text-[0.24rem] leading-[0.32rem] mt-[0.24rem]"
      ...
      <div class="flex lg:hidden pagination-bar gap-x-[0.09rem] relative">
        <div 
          class="bar w-[0.48rem] h-[0.04rem] bg-[#222]/20" 
          v-for="(_, idx) in slides" 
          :key="idx"
          @click="handleSlideTo(idx)"
        ></div>
        <div 
          class="active-bar w-[0.48rem] h-[0.04rem] bg-[#222] top-0 absolute duration-300"
          :style="activePaginationLeft"
        ></div>
      </div>
    </div>
