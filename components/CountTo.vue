  ...
  <div  ref="targetElement"
   class="count-to inline-flex flex-col lg:items-center select-none"
    ...
    <div v-if="flag" ref="valueRef" class="value-wrap flex font-miLight
        text-[0.48rem] leading-[0.64rem] h-[0.64rem] mb-[0.08rem]
        lg:leading-[0.48rem] lg:h-[0.48rem] lg:mb-[0.3rem]" :class="valueClass">
      <div class="value relative flex overflow-hidden h-full">
        <div class="duration-1000 ease-in-out inline-flex flex-col h-full text-center" v-for="(val, idx) in countList"
          :key="val" :style="{ transform: `translate(0px, ${transformList[idx]}px)` }" :data-num="val">
          <span v-if="val == '0'"  class="inline-block leading-[inherit]">0</span>
          <template v-else>
            <div v-for="i in (Number(val) + 1)" :key="i"
              :style="{ textAlign: i == 2 ? (idx == 0 ? 'right' : 'center') : 'center',height:`${transformUnitHeight}px`}">{{ i - 1 }}</div>
          </template>
        </div>
      </div>
    ...
    <div :class="labelClass" class="label text-[0.24rem] leading-[0.29rem] 
        lg:leading-[0.24rem] lg:text-[0.22rem] text-[#999]">{{ label }}</div>
  </div>
</template>
...
let transformUnit = 64.00;
const transformList = ref<number[]>([]);
const transformUnitHeight = ref(64.00);
